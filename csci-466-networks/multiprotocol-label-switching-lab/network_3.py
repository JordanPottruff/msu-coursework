...
class MPLSFrame:
    ...
    def __init__(self, label, packet, priority):
        self.label = label
    ...
    def from_byte_S(self, byte_S):
        ...
        priority = byte_S[MPLSFrame.label_length]
        packet = byte_S[MPLSFrame.label_length + 1 : ]
...
class NetworkPacket:
    ...
    def __init__(self, dst, data_S, priority=0):
        ...
        self.data_S = data_S
        self.priority=priority
        #TODO: add priority to the packet sclass
...
class Host:
    ...
    def udt_send(self, dst, data_S, priority=0):
        ...
        print('%s: sending packet "%s" with priority %s' % (self, pkt, priority))
    ...
    def udt_receive(self):
        ...
        pkt_S = fr.data_S
        #pkt_priority = fr.priority
        print('%s: received packet %s' % (self, pkt_S))#, pkt_priority))
...
class Router:
    ...
    def sort_out(self):
        ...
        for i in range(len(self.intf_L)):
            fr_S=None
            fr_S=self.intf_L[i].get('out')
            if fr_S is None:
                continue # no frame to process yet
            sorted_pkts=[]
            while fr_S!=None:
                sorted_pkts.append(fr_S)
                fr_S=self.intf_L[i].get('out')
            for j in sorted_pkts:
                #print("j -", j)
                if NetworkPacket.from_byte_S(LinkFrame.from_byte_S(j).data_S).priority == '1':
                    self.intf_L[i].put(j,'out',True)
                    sorted_pkts.remove(j)
                    prior1+=1
            for j in sorted_pkts:
                if j != None:
                    self.intf_L[i].put(j,'out',True)
        ...
        if prior1!=0 or prior0!=0:
            print("%s: prior1:%d prior0:%d"%(self.name,prior1,prior0))
    ...
    def process_network_packet(self, pkt, i):
        ...
        m_fr = MPLSFrame(label, pkt, pkt.priority)
    ...
    def process_MPLS_frame(self, m_fr, i):
        ...
        try:
            ...
            if str(self.decap_tbl_D['lbl']) == str(m_fr.label):
                netPacket = m_fr.packet
                fr = LinkFrame('Network', netPacket)
                intf = self.decap_tbl_D['outI']
            else:
                outIntf = None
                for entry in self.frwd_tbl_D:
                    if str(entry['inL']) == str(m_fr.priority) and entry['inI'] == i:
                        intf = entry['outI']
                        m_fr.label = entry['outL']
                        break
