...
class Host:
    ...
    def udt_send(self, dst_addr, data_S):
        if len(data_S) > self.out_intf_L[0].mtu - NetworkPacket.dst_addr_S_length:
            p = NetworkPacket(dst_addr, data_S[0:self.out_intf_L[0].mtu - NetworkPacket.dst_addr_S_length])
            self.out_intf_L[0].put(p.to_byte_S()) #send packets always enqueued successfully
            print('%s: sending packet "%s" on the out interface with mtu=%d' % (self, p, self.out_intf_L[0].mtu))
            self.udt_send(dst_addr,data_S[self.out_intf_L[0].mtu - NetworkPacket.dst_addr_S_length:])
        else:
