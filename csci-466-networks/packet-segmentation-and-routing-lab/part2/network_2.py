                ...
                while not done:
                    ...
                    if p_next.pack_id == p.pack_id:
                        ...
                        # otherwise use the data and exit
                            ...
                            self.received_buf.append(NetworkPacket.from_byte_S(pkt_S))
                            done = True
        ...
        while True:
            ...
            if(self.stop):
                ...
                for packet in self.received_buf:
                    packet = NetworkPacket.from_byte_S(packet.data_S)
                ...
                if pkt_S is not None:
                    ...
                    if len(pkt_S) > self.out_intf_L[i].mtu:
                        ...
                        while len(pkt_S) > 0:
                            ...
                            self.out_intf_L[i].put(p_next.to_byte_S(), True)
                            print('%s: forwarding packet "%s" from interface %d to %d with mtu %d' \
                                  % (self, p_next, i, i, self.out_intf_L[i].mtu))
